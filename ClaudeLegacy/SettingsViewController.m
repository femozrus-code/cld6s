...
NSString *const kSelectedAltIconKey = @"BootMenu.SelectedAltIcon"; // nil/absent = primary icon
NSString *const kCustomNameKey = @"BootMenu.CustomName";
...
- (void)viewDidLoad {
    ...
    [nameField addTarget:self action:@selector(nameFieldChanged) forControlEvents:UIControlEventEditingChanged];
    ...
    UIStackView *headerRow = [[UIStackView alloc] initWithArrangedSubviews:@[title, closeButton]];
    headerRow.axis = UILayoutConstraintAxisHorizontal;
    headerRow.distribution = UIStackViewDistributionEqualSpacing;

    UIStackView *stack = [[UIStackView alloc] initWithArrangedSubviews:@[
        headerRow, iconLabel, iconRow, nameLabel, nameField, noteLabel
    ]];
    stack.axis = UILayoutConstraintAxisVertical;
    stack.spacing = 10;
    [stack setCustomSpacing:24 afterView:headerRow];
...
- (void)iconTileTapped:(UIButton *)sender {
    ...
    [[UIApplication sharedApplication] setAlternateIconName:iconName completionHandler:^(NSError * _Nullable error) {
        if (error) {
            NSLog(@"[settings] setAlternateIconName failed: %@", error);
        }
    }];
